...
def ask_claude_review(stats: dict) -> str:
...
1. Что говорят цифры о качестве системы — честно, без приукрашивания.
2. Если видно, что какой-то тип сетапа (setup_type) заметно слабее других —
   укажи это конкретно.
3. Предложи 2-3 КОНКРЕТНЫХ параметра из StrategyParams (strategy.py), которые
   стоит попробовать изменить, и в какую сторону — с обоснованием, почему
   именно это может помочь, а не общие фразы.
